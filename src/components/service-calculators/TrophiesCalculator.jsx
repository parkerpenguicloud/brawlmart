...
import { calculateTrophyPrice } from "../PriceCalculator";
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-sm font-bold text-pink-400">{value.toLocaleString()} 🏆</span>
      </div>
      <input
        type="range"
        min={min}
...
export default function TrophiesCalculator({ onPriceChange }) {
    ...
    <div className="space-y-8">
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-6">
        ...
        <TrophyInput
          ...
          value={current}
          onChange={v => { setCurrent(v); if (desired <= v) setDesired(Math.min(v + STEP, MAX)); }}
          min={0}
          max={MAX - STEP}
        />
        <TrophyInput
        ...
        <div className="text-center mb-6">
          ...
          {price > 0 ? (
            <div className="flex items-baseline justify-center gap-1">
              ...
              <span className="text-5xl font-black bg-gradient-to-r from-pink-400 to-rose-400 bg-clip-text text-transparent">
                {price.toFixed(2)}
              </span>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={stat.label} className="text-center p-3 rounded-xl bg-white/[0.03]">
                <stat.icon className="w-4 h-4 text-pink-400 mx-auto mb-1" />
